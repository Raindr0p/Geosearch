...
public:
  ...
  void printPointPos() { cout << "(" << x << "," << y << ")" << endl; }
...
public:
  void initFormula(string f) {
    ...
    if (arr[0] == 'y' && arr[1] == '=') {
      ...
      cout << "The formula can not be initialized.";
  }
  ...
  void printFormula() {
    if (power == -1)
      ...
      cout << "(" << coefficient2 << ")"
           << "x^2+(" << coefficient1 << ")x"
           << "+(" << algebraic_coefficient << ")" << endl;
  }

private:
  char arr[50];
  double coefficient1, coefficient2, algebraic_coefficient, power,
      inverse_coffcient;
};
